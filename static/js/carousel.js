    ...
    try {
        ...
        const animation = {
            ...
            animate(state, imageSrc, prevSrc) {
                ...
                modalBg.style.background = `left top / cover url(${prevSrc}) no-repeat`;
        ...
        function addModalEvents(state, images, src) {
        ...
        if (webpSupport) {
            ...
            addModalEvents(state, allWbpSrc, 'srcset');
        } else {
            const state = initialState;
            initialActiveImageAndSetImage(state, allAsideImg, 'src');
            addModalEvents(state, allAsideImg, 'src');
        }
    } catch (error) {
        console.warn(error);
    }
